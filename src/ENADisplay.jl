...
function Makie.plot(ena::ENAModel;
    ...
    subtraction::Bool=false, showprojection::Bool=false,
    markerscale::Real=1, linescale::Real=1, textscale::Real=1)

    # Tidying up
    if subtraction && isnothing(ena.groupVar)
        error("ENA subtraction plots require the ENAModel to have a groupVar.")
    end
