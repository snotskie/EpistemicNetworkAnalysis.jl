...
## And colorLinesBy argument like that too, something like that
...
function Makie.plot(ena::ENAModel;
    ...
    linewidth = ena.networkModel[!, :density]
    ...
    # Codes with labels
    scatter!(scene, ena.codeModel[!, :fit_x], ena.codeModel[!, :fit_y], markersize=0.1*ena.codeModel[!, :density]*markerscale)
