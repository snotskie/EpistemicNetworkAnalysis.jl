...
function rotate!(
    ...
    if length(offTopicNodes) > 0
        ...
        if length(offTopicNodes) == 1
            ...
            @debug numSVDDims
            embedding = similar(model.embedding, numSVDDims+2) # plus 1 for the final control, plus 1 for the topic axis
            embedding[1:numSVDDims, edgeIDs] = svd[1:numSVDDims, :]
            embedding[1:numSVDDims, :eigen_value] = eigvals(pca)[1:numSVDDims]
            embedding[1:numSVDDims+1, :label] = ["OffTopic$(i)" for i in 1:numSVDDims+1]
            for edgeID in edgeIDs
                muOffTopic = mean(model.nodes[offTopicRows, edgeID])
                embedding[numSVDDims+1, edgeID] = muOffTopic
            end
        end
    end
...
function defaultplotkwargs(
        ...
        x::Int=1 + length(filter(label -> startswith(label, "OffTopic"), model.embedding.label)),
        y::Int=length(filter(label -> startswith(label, "OffTopic"), model.embedding.label)) > 0 ? 1 : 2,
        kwargs...
    ) where {R<:AbstractTopicRotation, M<:AbstractLinearENAModel{R}}
