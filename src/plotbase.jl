...
function plot_units!(p::Plot, ena::AbstractENAModel, displayRows::Array{Bool,1};
    ...
    if !isnothing(showTrajectoryBy) && count(displayRows) > 5
        ...
        if showTrajectoryBy in Symbol.(names(smoothingData))
            ...
            # xspline = Spline1D(ts, xs, k=5, bc="nearest")
            # yspline = Spline1D(ts, ys, k=5, bc="nearest")
            bspline = ParametricSpline(ts, ps, k=5, bc="nearest")
